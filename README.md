...
> **当前开放范围（2026-09-14）**：主界面开放「看图找规律」「看视频」「我的」。
> 底部选项卡的「课程 / 进度」和首页其余入口都保留可见但已去色置灰、点不进去；
> 「看图找规律」的入口卡片与选项卡带一圈沿边框转动的光辉。
> 答题过程中按返回键会被拦下：先答对一道一位数乘法才能退出，答错或 10 秒
> 超时都留在答题页；一局做完到成绩页则不再拦截。
> **看视频 / 播放页完全不拦返回键**，随时退出。
...
| 首页 | 问候 + 「看图找规律」入口（带转圈光辉）；搜索、本周精选 Banner、继续学习、为你推荐置灰保留、不可点 |
| 底部导航 | 「看图找规律」「看视频」「我的」开放，「看图找规律」带转圈光辉；「课程 / 进度」置灰可见、不可进入 |
| 看视频 | 展示「我的」里建好的视频分类，点分类进去看视频列表，点视频即全屏播放；返回键不受限制 |
| 视频管理 | 「我的 → 我的视频」：新建 / 重命名 / 删除分类，往分类里加视频（粘贴 http(s) 链接，或从本机选文件——文件会复制进 App 私有目录，不怕缓存被清） |
...
├─ state/                       # Riverpod providers（认证 / 学习进度 / 目录 / 主题 / 视频库）
├─ shared/
...
└─ features/                    # 按业务拆分的页面
    ...
    ├─ home/  catalog/  course/
    ├─ lesson/  quiz/
    ├─ pattern_quiz/            # 「看图找规律」（models + bank + screen + celebration + sfx + exit_gate）
    ├─ video/                   # 「看视频」（library + category + player + manage + 本机文件导入）
    └─ progress/  profile/
...
题库按 `PatternAgeGroup` 分 6 档：2–3 岁（30 题）/ 3–4 岁（30 题）/
5–6 岁（57 题）/ 7–8 岁（54 题）/ 9–10 岁（54 题）/ 100 岁（47 题），共 272 题，
每档都够抽 20 局以上不重样。

> 「100 岁」是暂存分类：**交替题**在 4 格里规律只重复一次，孩子没法归纳，
> 一律挪到这里（题号第 2 段为 `100`，如 `PT.100.SEQ.ALT.red-blue`），
> 不再进入其它年龄段的出题池。短名以 `-alt` 结尾的那批（属性题里的
> 「粗、细一个隔一个」等）连「100 岁」也不留，已整批删除。
题型参考常见 IQ / 图形推理测试的规律类别：交替、循环、数量增减、大小、深浅、
方向旋转、数列（等差 / 等比 / 平方 / 立方 / 质数 / 交错 / 复合递推 / 对称）、
图形旋转、球的旋转（皮球上的四瓣花纹，四分之一圈 / 八分之一圈）、
时针的旋转（每次走一 / 两 / 三个小时或半小时）、数量与颜色二维规律，
以及「数字 ↔ 数量 / 骰子 / 图形个数」配对。
